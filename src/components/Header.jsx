        ...
        <li className='relative group'>
            {item.type === "link" ? (
                <>
                    ...
                    <span className='cursor-pointer text-blue-500 absolute transtion-all duration-500 font-bold right-0 top-0 group-hover:right-[90%] opacity-0 group-hover:opacity-100  '>/</span>
                </>
            ) : (
        ...
        <section className='sticky top-0 left-0 right-0 z-50 '>
            <header className='container mx-auto px-5 flex justify-between py-4 items-center'>
                ...
                <div className={`${navIsVisible ? "right-0" : "-right-full"} transition-all duration-300 mt-[56px] lg:mt-0 bg-dark-hard lg:bg-transparent z-[49] flex flex-col w-full lg:w-auto justify-center lg:justify-end lg:flex-row fixed top-0 bottom-0  lg:static gap-x-9 items-center`}>
                    ...
                    <button className='mt-5 lg:mt-0 border-2 border-blue-500 px-6 py-2 rounded-full text-blue-500 font-semiblod hover:bg-blue-500 hover:text-white transition-all duration-300'>Sign in</button>
                </div>
            </header>
        </section>
    )
